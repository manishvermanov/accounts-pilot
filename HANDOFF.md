...
- **Airbnb** — first-pass handlers added (gated on `self.ota == "airbnb"`) for the 'become a host' single-listing wizard: `_fill_airbnb_structure` (property-type card grid), `_fill_airbnb_privacy_type` (entire place / room / shared room), `_fill_airbnb_location` (manual address — country first, then Street/Apt/City/State/ZIP), `_fill_airbnb_floor_plan` (`_airbnb_set_stepper` drives the Guests/Bedrooms/Beds/Bathrooms +/- steppers via `data-testid='stepper-floorPlan<Field>-…'`; counts derived by `_airbnb_floor_counts`, clamped to 16 guests / 50 else), `_fill_airbnb_amenities` (toggles tiles mapped from facilities by `_airbnb_amenity_labels`), `_fill_airbnb_title`, `_fill_airbnb_description`, `_fill_airbnb_price` (cheapest room base_rate). Cards/steppers/tiles BLOCK the LLM (`mmt_basic`) and let Next advance; every handler is body-text + URL gated and degrades to the LLM walker on any unmapped page. 18 unit tests in `tests/test_airbnb.py`; offline sim wired into `simulate.py` `_FLOWS` (location/title/description/price). **Selectors still need one live run to tighten** — run to each step, then read `data/logs/airbnb.log` + `data/training/airbnb/` and paste any page that stalls so the exact `data-*` ids get locked in (same loop used for MMT/Agoda/Expedia). Airbnb is heavily bot-protected, so login + any CAPTCHA stays a human gate.
